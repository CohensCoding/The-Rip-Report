...
function StatusChip({ status }: { status: LegacyRelease["status"] }) {
  ...
  return (
    <span className="inline-flex items-center rounded-full bg-violet-500/20 px-2 py-0.5 text-[10px] font-medium tracking-wide text-violet-400 uppercase">
      Announced
    </span>
  );
}
      ...
      <div className="pl-1">
        ...
        <div className="space-y-2.5 p-5">
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center rounded-full border border-zinc-800 bg-zinc-900/60 px-2 py-0.5 text-[10px] font-medium tracking-wide text-paper/80 uppercase">
              {brandLabel}
            </span>
            <span
              className={cn(
                "inline-flex items-center rounded-full border border-zinc-800 bg-zinc-900/60 px-2 py-0.5 text-[10px] font-medium tracking-wide uppercase",
                sport.textClass,
              )}
            >
              {sport.label}
          ...
          <div className="tabular text-xs text-zinc-500">{formatReleaseDate(release.releaseDate)}</div>

          <p className="line-clamp-2 text-xs italic text-zinc-300 sm:text-sm">{release.tagline}</p>
        </div>
      </div>
    </Link>
  );
